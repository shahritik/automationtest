...
public class Product6_Department_Brand extends MainMethod{

	//check if only the department related to a particular brand are shown
	
	public static void department_brand() throws InterruptedException, IOException
	{
		//click on item
		...
		Thread.sleep(3000);
		//check if initially the department tab should be empty since no brand is selected
		driver.findElement(By.xpath("//*[@id=\"rail\"]/div/div/div/div/div/div[6]/div/div")).click();
		Thread.sleep(3000);
		//take the screenshot for the empty department dropdown
		screenshotPath=TakeScreenshot.getScreenshot(driver, "ADA");
		List<WebElement> list = driver.findElements(By.xpath("//*[@id=\"rail\"]/div/div/div/div/div/div[6]/div/div/div[2]/child::div"));
		if(list.size()==1)
		{
			test.log(LogStatus.PASS, "最初は、部門のドロップダウンは空です", test.addScreenCapture(screenshotPath));
		}
		else
		{
			test.log(LogStatus.FAIL, "最初は、部門のドロップダウンは空です", test.addScreenCapture(screenshotPath));
		}
		Thread.sleep(3000);
		//close the department drop down
		driver.findElement(By.xpath("//*[@id=\"rail\"]/div/div/div/div/div/div[6]/div/div")).click();
		Thread.sleep(3000);
		//selecting the brand so that department is displayed
		...
		for(int i=2;i<3;i++)
		{
		    ...
		    //ASSERTION CONDITION
		    //get the list of department from CRXDE and compare it with the list shown on Asset Search Common Screen
		    if(correctareadisplayed(el)==true)
		    {
		    	test.log(LogStatus.PASS, "ブランドに関連した部門のみ表示される", test.addScreenCapture(screenshotPath));
		    }
		    else
		    {
		    	test.log(LogStatus.FAIL, "ブランドに関連した部門のみ表示される", test.addScreenCapture(screenshotPath));
		    }
		}
	...
	private static boolean correctareadisplayed(WebElement el) throws InterruptedException {
		...
		//clicking on the department drop down
	    driver.findElement(By.xpath("//*[@id=\"rail\"]/div/div/div/div/div/div[6]/div/div")).click();
	    Thread.sleep(3000);
	    ...
	    Thread.sleep(500);
	    driver.get("http://13.113.230.58:4502/libs/cq/search/content/querydebug.html?_charset_=UTF-8&query=path%3D%2Fetc%2Ftags%2Fadastria%2Fitem%2Fmanage_department%0D%0Aproperty%3D.%2FbrandId%0D%0Aproperty.value%3D"+(brandId)+"%0D%0Ap.limit%3D-1");
		Thread.sleep(3000);
		if(k==0)
		{
