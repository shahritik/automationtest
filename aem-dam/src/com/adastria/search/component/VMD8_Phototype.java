...
//filter the assets as per the phototype and check if the assets are correctly displayed
public class VMD8_Phototype extends MainMethod{
	public static void phototype() throws InterruptedException, IOException
	{
		...
		Thread.sleep(2000);
		driver.findElement(By.xpath("//*[@id=\"rail\"]/div/div/div/div/div/div[8]/div[2]/div")).click();
		Thread.sleep(2000);
		List<WebElement> list = driver.findElements(By.xpath("//*[@id=\"rail\"]/div/div/div/div/div/div[8]/div[2]/div/div[2]/child::div"));
		System.out.println(list.size());
		for(int i=0;i<list.size();i++)
		{
			WebElement el=driver.findElement(By.xpath("//*[@id=\"rail\"]/div/div/div/div/div/div[8]/div[2]/div/div[2]/div["+(i+1)+"]"));
			if(el.getText().equals("売場写真")==true)
			{
				el.click();
			}
		}
		...
		{
			test.log(LogStatus.PASS, "選択した写真種別のアセットが表示される", test.addScreenCapture(screenshotPath));
		}
		else
		{
			test.log(LogStatus.FAIL, "選択した写真種別のアセットが表示される", test.addScreenCapture(screenshotPath));
		}
	}
	...
	private static boolean correctassetdiplayed() throws InterruptedException 
	{
		...
		inner1:for(int j=0;j<2;j++)
		{
			...
			if(value.equals("売場写真")==false && header.equals("写真種別")==false)
			{
				continue inner2;
			}
			else if(value.equals("売場写真")==false || header.equals("写真種別")==false)
			{
				return false;
			}
			else if(value.equals("売場写真")==true || header.equals("写真種別")==true)
			{
