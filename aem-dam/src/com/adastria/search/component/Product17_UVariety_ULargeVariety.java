...
public class Product17_UVariety_ULargeVariety extends MainMethod{

	//create a function to check if the variety drop down is empty initially
	...
	public static void variety_largevariety() throws InterruptedException, IOException
	{
		...
		List<WebElement> list = driver.findElements(By.xpath("//*[@id=\"rail\"]/div/div/div/div/div/div[17]/div/div/div[2]/child::div"));
		if(list.size()==1)
		{
			test.log(LogStatus.PASS, "最初は、統一品種のドロップダウンは空です", test.addScreenCapture(screenshotPath));
		}
		else
		{
			test.log(LogStatus.FAIL, "最初は、統一品種のドロップダウンは空です", test.addScreenCapture(screenshotPath));
		}
		Thread.sleep(3000);
		...
		List<WebElement> list1 = driver.findElements(By.xpath("//*[@id=\"rail\"]/div/div/div/div/div/div[16]/div/div/div[2]/child::div"));
		...
		for(int i=1;i<2;i++)
		{
		    ...
		    {
		    	test.log(LogStatus.PASS, "統一大品種に関連した品種のみ表示される", test.addScreenCapture(screenshotPath));
		    }
		    else
		    {
		    	test.log(LogStatus.FAIL, "統一大品種に関連した品種のみ表示される", test.addScreenCapture(screenshotPath));
		    }
		}
	...
	private static boolean correctareadisplayed(WebElement el) throws InterruptedException {
	    ...
	    driver.findElement(By.xpath("//*[@id=\"rail\"]/div/div/div/div/div/div[17]/div/div")).click();
	    Thread.sleep(3000);
		List<WebElement> list1 = driver.findElements(By.xpath("//*[@id=\"rail\"]/div/div/div/div/div/div[17]/div/div/div[2]/child::div"));
		System.out.println(list1.size());
		String ar[]=new String[list1.size()-1];
		for(int i=0;i<list1.size();i++)
		{
			WebElement el1=driver.findElement(By.xpath("//*[@id=\"rail\"]/div/div/div/div/div/div[17]/div/div/div[2]/div["+(i+1)+"]"));
	    ...
	    Thread.sleep(500);
	    driver.get("http://13.113.230.58:4502/libs/cq/search/content/querydebug.html?_charset_=UTF-8&query=path%3D%2Fetc%2Ftags%2Fadastria%2Fintegration_type%0D%0Aproperty%3D.%2FintegrationLargeTypeId%0D%0Aproperty.value%3D"+(itemLargeTypeId)+"%0D%0Ap.limit%3D-1");
		Thread.sleep(3000);
		if(k==0)
		{
