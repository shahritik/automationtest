...
public class StaffBoard1_EC extends MainMenu_Test{
	
	static String str;

	public static void staffboard_ec() throws InterruptedException, IOException
	{
		...
		Thread.sleep(2000);
		//opening the EC　用着画　dropdown
		driver.findElement(By.xpath("//*[@id=\"rail\"]/div/div/div/div/div/div[2]/div/div/div[1]")).click();
		Thread.sleep(1000);
		//checking the EC用着画 checkbox
		driver.findElement(By.xpath("//*[@id=\"rail\"]/div/div/div/div/div/div[2]/div/div/div[2]/div/div/div/input")).click();
		//getting the string associated with the checkbox
		str=driver.findElement(By.xpath("//*[@id=\"rail\"]/div/div/div/div/div/div[2]/div/div/div[2]/div/div/div/label")).getText();
		...
		{
			test.log(LogStatus.PASS, "EC用着画のアセットが表示されます", test.addScreenCapture(screenshotPath));
		}
		else
		{
			test.log(LogStatus.FAIL, "EC用着画のアセットが表示されます", test.addScreenCapture(screenshotPath));
		}
	}
	private static boolean correctassetdiplayed() throws InterruptedException 
	{
		...
		if(element.size()!=0)
		{
			inner1:for(int j=0;j<element.size();j++)
			{
				...
				if(value.equals("EC用着画")==false && header.equals("ada-staffboard")==false)
				{
					continue inner2;
				}
				else if(value.equals("EC用着画")==false && header.equals("ada-staffboard")==true)
				{
					return false;
				}
				else if(value.equals("EC用着画")==true && header.equals("ada-staffboard")==true)
				{
